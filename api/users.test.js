...
import { getUsers, getUser, userDomain, allUserRoute } from './users';
...
describe('users api', () => {
  ...
  describe('getUsers', () => {
    it('throws error if response is 500', async () => {
      nock(userDomain).get(allUserRoute).reply(500);
    ...
    it('returns list of users if response is ok', async () => {
      nock(userDomain).get(allUserRoute).reply(200, mockUsers);

      expect(await getUsers()).toEqual(mockUsers);
    });
  });
